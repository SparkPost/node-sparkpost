...
SparkPost.prototype.request = function( options, callback ) {
  ...
  request(options, function(err, res, body) {
    ...
    // Error with the proxy request
    if(err) {
      return callback(err, res);
    }

    // Error with the origination request
    var twoHundredRegEx = /^2[0-9]{2}$/;
    if(!twoHundredRegEx.test(res.statusCode)) {
      var error = new Error('A ' + res.statusCode + ' was returned by the API. Please see the errors property for details.');
      error.name = 'SparkPostError';
