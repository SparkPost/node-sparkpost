...
var sendgrid = function(username, apiKey, options) {
  ...
  //TODO: Handle protocol, host, and port
  this.client = new SparkPost(apiKey, configOptions);
};
...
sendgrid.prototype.send = function(payload, callback) {
  ...
  this.client.transmission.send(translated, callback /*function (err, res){
    if (err) {
      callback(err);
    } else {
      callback(null, res);
