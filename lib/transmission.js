 ...
 *
 *  @param meta object Object of key-value pairs to be applied to the transmission level metadata
 */
...
transmission.prototype.setSubstitutionData = function(subs) {
  if (subs.constructor !== Object) {
    throw new Error('Substitution data must be an object');
  } else {
    this.model.substitutionData = subs;
  }
...
transmission.prototype.setFrom = function(from) {
  if (typeof from === 'string') {
    ...
    if (from.hasOwnProperty('email') === false) {
      throw new Error('From must at least have an email field when passed as an object');
    } else {
      this.model.content.from = from;
    }
  } else {
    throw new Error('From must be an object or string');
  }
};
...
transmission.prototype.setRfc2822Content = function(rfc) {
  if (typeof rfc !== 'string') {
    throw new Error('RFC 2822 encoded content must be a string');
  } else if (this.model.content.hasOwnProperty('html') || this.model.content.hasOwnProperty('plaintext')) {
    throw new Error('You cannot use RFC-2822 encoded content after providing HTML or Plain Text content');
  } else {
    this.model.content.rfc2822 = rfc;
  }
...
transmission.prototype.useStoredTemplate = function(templateID) {
  if (typeof templateID !== 'string') {
    ...
    throw new Error('You cannot use a stored template if you have already provided HTML or Plain Text content');
  } else if (this.model.content.hasOwnProperty('rfc2822')) {
