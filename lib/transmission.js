...
var Transmission = function(params) {
  ...
  this.model.list_name = params.recipientList;
  this.model.content.template_id = params.template;

  this.model.options.open_tracking = params.openTracking || true;
  this.model.options.click_tracking = params.clickTracking || true;
  this.model.content.use_draft_template = params.useDraftTemplate || false;
};
...
var fetch = function(config, transmissionID, callback) {
  var cb = callback
    , options = {
      url: config.schema + '://' + config.host + config.baseUrl,
      headers: {
...
Transmission.prototype.usePublishedTemplate = function() {
  this.model.use_draft_template = false;
};
