...
Transmission.prototype.send = function(callback) {
  var options = {
    url: this.options.schema + '://' + this.options.host + (this.options.port ? ':' + this.options.port : '') + this.options.baseUrl,
    json: this.model,
    headers: {
...
var fetch = function(config, transmissionID, callback) {
  var cb = callback
    , options = {
      url: config.schema + '://' + config.host + (config.port ? ':' + config.port : '') +  config.baseUrl,
      headers: {
